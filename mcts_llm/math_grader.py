...
def _test_extract_answer():
    text = "\\boxed{3\\sqrt{3}}"
    print(extract_answer(text, "math", use_last_number=False))
    print(choice_answer_clean("\mathrm{(D)\}1,008,016"))
    # should output a dict

    answer = r"\\frac{1}{2}"
    print(f"{answer} == {extract_answer(text, 'math', use_last_number=False)}")
    print(answer == extract_answer(text, "math", use_last_number=False))
    print(r"\frac{1}{2}" == "\\frac{1}{2}") # True
    print("\frac{1}{2}" == "\\frac{1}{2}") # False

    problem = {"problem": "What is the distance between the two intersections of $y=x^2$ and $x+y=1$?", "solution": "To find the $x$-coordinates of the intersections, substitute $x^2$ for $y$ in $x+y=1$ and solve for $x$, resulting in  \\begin{align*}\nx+x^2&=1 \\\\\n\\Rightarrow \\qquad x^2+x-1&=0 \\\\\n\\Rightarrow \\qquad x&=\\frac{-1\\pm\\sqrt{1+4}}2=\\frac{-1\\pm\\sqrt5}2\\\\\n\\end{align*}Using each of these coordinates to solve for $y$ gives us the intersections at $\\left(\\frac{-1+\\sqrt5}2,\\frac{3-\\sqrt5}2\\right)$ and $\\left(\\frac{-1-\\sqrt5}2,\\frac{3+\\sqrt5}2\\right)$.  Using the distance formula, we have \\begin{align*}\n&\\sqrt{ \\left(\\frac{-1+\\sqrt5}{2}-\\frac{-1-\\sqrt5}{2}\\right)^2 + \\left(\\frac{3-\\sqrt5}2-\\frac{3+\\sqrt5}2\\right)^2 }\\\\\n&\\qquad=\\sqrt{\\left(\\frac{2\\sqrt5}2\\right)^2 + \\left(-\\frac{2\\sqrt5}2\\right)^2}\\\\\n&\\qquad=\\sqrt{ 2\\sqrt5^2 }\\\\\n&\\qquad=\\boxed{\\sqrt{10}}.\n\\end{align*}", "answer": "\\sqrt{10}", "subject": "Algebra", "level": 5, "unique_id": "test/algebra/487.json"}


    print(extract_answer(problem["solution"], "math", use_last_number=False))
    print('\\sqrt{10}' == extract_answer(problem["solution"], "math", use_last_number=False))
...
def _test_math_equal():
    ...
    print(math_equal('7/16', '\\frac{7}{16}', timeout=True))
    print(math_equal('0.4375', '\\frac{7}{16}', timeout=True))
